...
def multicast(index, currentFile):
    for s in downloadInfos[currentFile]:
        (bitvect, grpID) = downloadInfos[currentFile][s]
        if(grpID==myGroupID):
            msg = "HAVE_PIECE"+SEPAERATOR+currentFile+SEPAERATOR+str(index)+SEPAERATOR+str(blockNumber)+SEPAERATOR+byteData+DELIMITER


def broadcast(index, currentFile):
    for s in downloadInfos[currentFile]:
        msg = "PIECE_DONE"+SEPAERATOR+currentFile+SEPAERATOR+str(index)+DELIMITER
...
def pieceRequestOrdering(filename, currentFile):
    ...
    for s in tempList:
        if s in downloadInfos[filename]:
            (bitvector, grpID) = downloadInfos[filename][s]
            if grpID==myGroupID:    #peer with the same group id should be on top of the request queue.
                index = 1
                for i in bitvector:
                    if i =='1':
...
def processRecvdMsg(data,s):
    ...
    if header == "RECEIVE_FILE":
        ...
        uploadInfos[filename][s] = (bitvector, grpID)


        bitvector =  returnBitVector(filename,pieces)
        data = "REPLY_HANDSHAKE"+SEPAERATOR+filename+SEPAERATOR+stringify(bitvector)+SEPAERATOR+str(myGroupID)+DELIMITER
        message_queues[s].put(data)
        ...
        grpID = int(temp[3])
        pieces = len(bitvector)
        downloadInfos[filename][s] = (bitvector, grpID)
        ...
        # print len(temp)
        # print len(byteData)
        bitvect = pieceBitVector[filename]
        if(bitvect[index-1]=="1"):
            return
        ...
        try:
            ...
            if(pieceStatus[index-1]==PIECE_SIZE/BLOCK_SIZE):
                numPiecesDownloaded = numPiecesDownloaded + 1
                multicast(index, filename)
    ...
    elif header=="PIECE_DONE":
        filename = temp[1]
        finished_index = int(temp[2])
        (bitvector, grpID) = downloadInfos[filename][s]
        index = 1
        newBitVector = ""
        for bit in bitvector:
            if(index==finished_index):
                newBitVector = newBitVector + "1"
            else:
                newBitVector = newBitVector + bit
        downloadInfos[filename][s] = (newBitVector, grpID)
...
def handShaking(peerList, currentFile):
    ...
    for peers in peerList:
        ...
        try:
            ...
            data = "HANDSHAKE"+SEPAERATOR+torrentInfo[currentFile]['name'] +SEPAERATOR+stringify(pieceBitVector[torrentInfo[currentFile]["name"]])+SEPAERATOR+str(myGroupID)+DELIMITER
...
class Example(wx.Frame):
    ...
    def __init__(self, parent, title):    
        ...
        sizer.Add(self.text3, pos=(5, 0), flag=wx.LEFT|wx.TOP, border=10)

        self.torrentFileText = wx.TextCtrl(self.panel)
        sizer.Add(self.torrentFileText, pos=(5, 1), span=(1, 3), flag=wx.TOP|wx.EXPAND, 
            border=5)
        ...
        sizer.Add(self.text4, pos=(4, 0), flag=wx.LEFT|wx.TOP, border=10)

        self.GroupText = wx.TextCtrl(self.panel)
        sizer.Add(self.GroupText, pos=(4, 1), span=(1, 1), flag=wx.TOP|wx.EXPAND, 
            border=5)
        ...
        sizer.Add(self.button1, pos=(5, 4), flag=wx.TOP|wx.RIGHT, border=5)
