...
def multicast(index, currentFile,blockNumber,byteData):
	...
	for s in myGroupList:
		...
		msg = "MULTICAST"+SEPAERATOR+currentFile+SEPAERATOR+str(index)+SEPAERATOR+str(blockNumber)+SEPAERATOR+byteData+DELIMITER
...
def processRecvdMsg(data,s):
	...
	if header == "RECEIVE_FILE":
		...
		data = "REPLY_HANDSHAKE"+SEPAERATOR+filename+SEPAERATOR+str(myGroupID)+SEPAERATOR+stringify(bitvector)+DELIMITER
		message_queues[s].put(data)
		...
		byteData = temp[4]
		print "Multicasting Message recieved for block = "+str(blockNumber) +"and piece = "+str(index) +"and file = "+filename
...
def handShaking(peerList, currentFile):
	...
	for peers in peerList:
		...
		try:
			...
			data = "HANDSHAKE"+SEPAERATOR+torrentInfo[currentFile]['name']+SEPAERATOR+str(myGroupID)+SEPAERATOR+stringify(pieceBitVector[torrentInfo[currentFile]["name"]])+DELIMITER
			print data	
